"""Карточка одного товара для списка с подсветкой по ТЗ:

- скидка > 25%        -> фон #23E1EF
- товара нет на складе -> фон голубой (#ADD8E6) [приоритетнее]
- цена снижена         -> старая цена зачёркнута красным, итоговая чёрным

Поиск изображений самодостаточный: фото из БД (например '1.jpg') ищется в
папке Sources рядом с модулем, без учёта регистра ('1.JPG').
"""
...
def resolve_image(photo):
    """Возвращает существующий путь к изображению или None.

    Учитывает подпапки (Sources/images) и регистр имени ('1.jpg' <-> '1.JPG').
    """
    if not photo:
        return None
...
def placeholder():
    """Путь к картинке-заглушке picture.png (если найдена)."""
...
class ProductCard(QFrame):
    ...
    def _apply_background(self, product):
        ...
        color = None
        if stock <= 0:
            color = "#ADD8E6"        # нет на складе -> голубой
        elif discount > 25:
            color = "#23E1EF"        # большая скидка
        if color:
